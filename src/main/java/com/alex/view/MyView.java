...
public class MyView implements Printable {
    ...
    private void printMethod(String table) throws SQLException {
        ...
        while (!isBack) {
            ...
            switch (consoleInput) {
                case "FA" -> {
                    System.out.println(controller.get(table).printTable());
                    findAll(controller.get(table));
                }
                case "FBI" -> {
                    System.out.println(controller.get(table).printTable());
                    findById(controller.get(table));
                }
                case "C" -> {
                    System.out.println(controller.get(table).printTable());
                    create(controller.get(table),models.get(table));
                }
                case "U" -> {
                    System.out.println(controller.get(table).printTable());
                    update(controller.get(table),models.get(table));
                }
            ...
            for (Field field : fields) {
                ...
                switch (field.getAnnotatedType().toString()) {
                    ...
                    case "java.lang.Double":
                        ...
                        break;
                    case "com.alex.model.models.Client":
                        field.set(model, controller.get("Client").findById(Integer.parseInt(consoleInput)));
                        break;
                    case "com.alex.model.models.Agency":
                        field.set(model, controller.get("Agency").findById(Integer.parseInt(consoleInput)));
                        break;
                    case "com.alex.model.models.AgencyAnimator":
                        field.set(model, controller.get("AgencyAnimator").findById(Integer.parseInt(consoleInput)));
                        break;
                    case "com.alex.model.models.AgencyType":
                        field.set(model, controller.get("AgencyType").findById(Integer.parseInt(consoleInput)));
                        break;
                    case "com.alex.model.models.Event":
                        field.set(model, controller.get("Event").findById(Integer.parseInt(consoleInput)));
                        break;
                    case "com.alex.model.models.EventAddress":
                        field.set(model, controller.get("EventAddress").findById(Integer.parseInt(consoleInput)));
                        break;
                    case "com.alex.model.models.OrderClient":
                        field.set(model, controller.get("OrderClient").findById(Integer.parseInt(consoleInput)));
                        break;
                    case "com.alex.model.models.Price":
                        field.set(model, controller.get("Price").findById(Integer.parseInt(consoleInput)));
                        break;
                    default:
                        break;
